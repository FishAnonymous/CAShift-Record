...
if __name__ == "__main__":
    ...
    #################################
    attack_path = f"/Dataset/attack-new"
    for index, dirname in enumerate(os.listdir(attack_path)):
        attack_id = dirname
        # attack_id = "CVE-2016-10033" ########### Remember to Change
        # create output directory
        os.makedirs(f"{log_path}/output/{dirname}", exist_ok=True)
        attack_log_path = os.path.join(attack_path, dirname)

        # for every scap file in attack_path
        for i, filename in enumerate(os.listdir(attack_log_path)):
            attack_scap_path = os.path.join(attack_log_path, filename)
            ...
            attack_df.to_csv(f"{log_path}/output/{dirname}/{attack_id}_{i}.csv", index=False)
            print(f"save log {i}")

        print(f"total {i+1} logs")
        print(f"test attack {dirname} saved")
