...
if __name__ == "__main__":
    ...
    ### For test normal ###
    ########################
    test_path = f"{log_path}/test-normal/all.scap"
    test_df = get_scap_dfs([test_path], "normal", -1)
    
    # create output directory
    os.makedirs(f"{log_path}/output/test-normal", exist_ok=True)
    ...
    while True:
        chunk_size = random.randint(40000, 60000)
        if total_size > record_size + chunk_size:
            ...
            chunk.to_csv(f"{log_path}/output/test-normal/normal_{log_number}.csv", index=False)
