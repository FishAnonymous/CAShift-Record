...
from sysdig_template import Sysdig # include template
...
def get_scap_dfs(scap_paths: List[str], attack: str, index: int) -> pd.DataFrame:
    ...
    for scap_path, scap_df in zip(scap_paths, scap_dfs):
        ...
        if all(item in scap_df.columns for item in ['timestamp', 'cpuid', 'syscall', 'args']):
            today = datetime.now().strftime('%Y-%m-%d')

            # if index == -1 then normal, else attack
            scap_df["label"] = '-' if attack == 'normal' else f"{attack}_{index}"
            ...
            scap_df["event"] = "<timestamp>" + scap_df['timestamp'].astype(str) + "; " + "<cpuid>" + scap_df['cpuid'].astype(str) + "; " + "<syscall>" + scap_df['syscall'].astype(str) +  "; " + "<args>" + scap_df['args'].astype(str) + ";"
...
if __name__ == "__main__":
    ...
    log_path = "/CloudAttackRecord/data_process/test"
    ...
    for i, chunk in enumerate(chunks):
        ...
        chunk = chunk.drop('args', axis=1)
        log_frame = chunk[["Label", "TimeStamp", "CPUID", "SysCall", "EventTemplate"]]
        log_frame.to_csv(f"{log_path}/output/normal/normal_{i}.csv", index=False)
    ...
    while True:
        ...
        if total_size > record_size + chunk_size:
            ...
            # normal_df["EventId"] = normal_df.groupby("Syscall").ngroup()
            log_frame = chunk[["Label", "TimeStamp", "CPUID", "SysCall", "EventTemplate"]]
            log_frame.to_csv(f"{log_path}/output/test/normal_{log_number}.csv", index=False)
    ...
    attack_id = "CVE_2019-5736" ########### Remember to Change
    # create output directory
    ...
    for i, filename in enumerate(os.listdir(attack_path)):
        ...
        attack_df = attack_df.drop('args', axis=1)
    
        log_frame = attack_df[["Label", "TimeStamp", "CPUID", "SysCall", "EventTemplate"]]
    ...
    # attack_df = get_scap_dfs([attack_path])
